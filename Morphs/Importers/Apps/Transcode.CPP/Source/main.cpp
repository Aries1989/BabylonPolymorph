...
R"(Usage: [FILE]... [OPTION]...
  ...
  --verbosity=[LEVEL]             The level of console output (default: default)
      ...
      quiet                             Importer/Exporter Warnings and Errors, plus limited Transcode.CPP logging
      default                           Importer/Exporter Warnings and Errors, plus typical Transcode.CPP logging
      verbose                           Detailed Importer/Exporter output, plus typical Transcode.CPP logging
  ...
  --exportoption=[KEY:VALUE]      A key-value pair that that will be passed through to Transcoders as an export option.
    ...
    void PrintTranscoderHelp(const std::string& importFormat, const std::string& exportFormat)
    {
        ...
        std::stringstream importHelp(Babylon::GetImportHelp(importFormat));
        ...
        std::stringstream exportHelp(Babylon::GetExportHelp(exportFormat));
    ...
    public:
        ...
        std::string GetArgument(const std::string& arg, const std::string& defaultValue = "")
        {
            ...
            {
                if (Babylon::Utils::BeginsWith(m_args[i] + "=", arg))
                {
    ...
    private:
        std::vector<std::string> ConvertWCharArgs(int argc, wchar_t* argv[])
        {
            ...
            {
                args.push_back(Babylon::Utils::WStringToString(argv[i], wcslen(argv[i])));
            }
            return args;
        }
    ...
    void SetVerbosity(const std::string& verbosity)
    {
        ...
        {
            // Show Important+ from Transcode.CPP and Warnings+ from dependencies
            Trace::SetAreaLevels(Trace::Output, Trace::Level::Warn);
        ...
        {
            // Show Info+ from Transcode.CPP and Warnings+ from dependencies
            Trace::SetAreaLevels(Trace::Output, Trace::Level::Warn);
        ...
        {
            // Show Info+ from Transcode.CPP and Important+ from dependencies
...
#endif
{
    ...
    {
        destination = Babylon::Utils::GetPathConsistentSlashes(Babylon::Utils::GetPathConcatenation(destination, ""));
    }
    ...
    for (auto& inputFilePath : inputFilePaths)
    {
        ...
        try
        {
            Babylon::Transcode(inputFilePath, format, importOptions, exportOptions, destination);
        }
